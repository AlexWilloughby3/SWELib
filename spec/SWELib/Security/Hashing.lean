...
theorem ipadByte_ne_opadByte : ipadByte ≠ opadByte := by native_decide
...
theorem hmacTruncated_size (alg : HashAlgorithm) (key : HmacKey)
    ...
    out.size = t := by
  unfold hmacTruncated at hout
  split at hout
  · case isTrue h =>
    simp at hout
    subst hout
    rw [ByteArray.size_extract, hmac_size]
    omega
  · case isFalse h => simp at hout
