...
namespace SWELibImpl.Cloud.GcpClient
...
def getInstance (cfg : GcpConfig) (zone inst : String) : IO GcpResult :=
  gcpGet cfg.apiBase cfg.accessToken
    s!"/compute/v1/projects/{cfg.projectId}/zones/{zone}/instances/{inst}"
    cfg.timeoutMs

/-- List GCS objects in a bucket, optionally filtering by prefix. -/
def listObjects (cfg : GcpConfig) (bucket : String) (pfx : String := "") : IO GcpResult :=
  let path := if pfx.isEmpty
    then s!"/storage/v1/b/{bucket}/o"
    else s!"/storage/v1/b/{bucket}/o?prefix={pfx}"
  gcpGet cfg.apiBase cfg.accessToken path cfg.timeoutMs

/-- Get metadata for a GCS object. -/
def getObject (cfg : GcpConfig) (bucket obj : String) : IO GcpResult :=
  gcpGet cfg.apiBase cfg.accessToken
    s!"/storage/v1/b/{bucket}/o/{obj}"
    cfg.timeoutMs
...
end SWELibImpl.Cloud.GcpClient
