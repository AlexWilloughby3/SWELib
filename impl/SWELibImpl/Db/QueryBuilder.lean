...
`SWELibImpl.Db.toSql` for execution.

## Usage

```lean
open SWELibImpl.Db.QueryBuilder
...
#eval SWELibImpl.Db.toSql q
-- "SELECT id, name FROM users WHERE id = active LIMIT 100"
```
-/

namespace SWELibImpl.Db.QueryBuilder
...
/-- Set the FROM clause. -/
def Builder.setFrom {Const : Type} (b : Builder Const) (items : List (FromItem Const)) :
    Builder Const :=
  { b with from_ := items }
...
def Builder.setWhere {Const : Type} (b : Builder Const) (cond : SqlCondition Const) :
    Builder Const :=
...
def Builder.setGroupBy {Const : Type} (b : Builder Const) (keys : List (SqlExpr Const)) :
    Builder Const :=
  { b with groupBy := keys }

/-- Set the HAVING clause. -/
def Builder.setHaving {Const : Type} (b : Builder Const) (cond : SqlCondition Const) :
    Builder Const :=
  { b with having := some cond }

/-- Add an ORDER BY item. -/
def Builder.addOrderBy {Const : Type} (b : Builder Const) (item : OrderByItem Const) :
    Builder Const :=
...
/-- Builder roundtrip: `build` then `toSql` is equivalent to `SWELibImpl.Db.toSql ∘ build`. -/
theorem builder_toSql_eq_pgclient {Const : Type} [ToString Const] (b : Builder Const) :
    b.toSql = SWELibImpl.Db.toSql b.build := by
  simp [Builder.toSql]

end SWELibImpl.Db.QueryBuilder
