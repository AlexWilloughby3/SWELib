...
namespace SWELibImpl.Security.JwtValidator
...
noncomputable def validateToken (token : String) (key : Jwk) (config : ValidationConfig)
    (now : NumericDate) : Except ValidationError Unit :=
  match parseCompact token with
  | .error _ => .error (.signature .invalidFormat)
  | .ok jwt => validate jwt key config now
...
noncomputable def isTokenExpired (token : String) (now : NumericDate) (clockSkew : Nat := 60) : Option Bool :=
  match parseCompact token with
  | .ok jwt => some (isExpired jwt now clockSkew)
  | .error _ => none
...
noncomputable def createSimpleToken (alg : JwtAlgorithm) (key : Jwk) (issuer subject : String)
    (expiresIn : Nat) (now : NumericDate) : Except CreationError String :=
  match createSimple alg key issuer subject expiresIn now with
  | .error e => .error e
  | .ok jwt => .ok (serializeCompact jwt)

end SWELibImpl.Security.JwtValidator