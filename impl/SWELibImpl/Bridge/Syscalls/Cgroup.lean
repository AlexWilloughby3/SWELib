...
/-- Bridge axiom: cgroup_create matches specification. -/
axiom cgroup_create_bridge (parent : Cgroup) (name : String) :
  cgroup_create parent name = cgroup_create_impl parent name

/-- Bridge axiom: cgroup_delete matches specification. -/
axiom cgroup_delete_bridge (cg : Cgroup) :
  cgroup_delete cg = cgroup_delete_impl cg

/-- Bridge axiom: cgroup_move_process matches specification. -/
axiom cgroup_move_process_bridge (cg : Cgroup) (pid : PID) :
  cgroup_move_process cg pid = cgroup_move_process_impl cg pid

/-- Bridge axiom: cgroup_set_limit matches specification. -/
axiom cgroup_set_limit_bridge (cg : Cgroup) (controller : CgroupController)
  (limit : CgroupLimit) :
  cgroup_set_limit cg controller limit = cgroup_set_limit_impl cg controller limit

/-- Bridge axiom: cgroup_get_usage matches specification. -/
axiom cgroup_get_usage_bridge (cg : Cgroup) (controller : CgroupController) :
  cgroup_get_usage cg controller = cgroup_get_usage_impl cg controller

/-- Bridge axiom: cgroup_enable_controller matches specification. -/
axiom cgroup_enable_controller_bridge (cg : Cgroup) (controller : CgroupController) :
...
axiom memory_limit_oom_axiom (cg : Cgroup) (limit : Nat)
  ...
  (h2 : ∃ usage, cgroup_get_usage cg .memory = .ok usage ∧ usage ≥ limit) :
