...
axiom pq_validate_with_timeout : ConnectionHandle → Nat → IO Bool
...
/-- Check if a connection has been idle for too long.
    ...
    Issue: https://github.com/SWELib/SWELib/issues/XXX -/
axiom pq_is_idle_too_long : ConnectionHandle → Nat → IO Bool
...
/-- Check if a connection has exceeded its maximum lifetime.
    ...
    Issue: https://github.com/SWELib/SWELib/issues/XXX -/
axiom pq_is_connection_expired : ConnectionHandle → Nat → IO Bool
...
/-- Perform a quick health check without query execution.
    ...
    Issue: https://github.com/SWELib/SWELib/issues/XXX -/
axiom pq_quick_health_check : ConnectionHandle → IO Bool
