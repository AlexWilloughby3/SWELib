...
axiom mmap_bridge (addr : Option SWELib.OS.VirtualAddress) (length : Nat)
    (prot : SWELib.OS.MemoryProtection) (flags : SWELib.OS.MappingFlags)
    ...
    if length = 0 then
      ...
      .error .ENOSYS  -- Would be actual implementation result

/-! ## munmap bridge axiom -/

-- TRUST: https://github.com/SWELib/SWELib/issues/XXX
/-- Bridge axiom for `munmap` system call. -/
axiom munmap_bridge (addr : SWELib.OS.VirtualAddress) (length : Nat) :
...
axiom mprotect_bridge (addr : SWELib.OS.VirtualAddress) (length : Nat)
    (prot : SWELib.OS.MemoryProtection) :
...
axiom brk_bridge (addr : SWELib.OS.VirtualAddress) :
    SWELib.OS.Memory.brk addr = .error .ENOSYS  -- Placeholder
...
axiom readProcMaps_bridge (pid : Nat) :
    SWELib.OS.Memory.readProcMaps pid = .error .ENOSYS  -- Placeholder

-- TRUST: https://github.com/SWELib/SWELib/issues/XXX
/-- Bridge axiom for reading `/proc/[pid]/oom_score`. -/
axiom readOOMScore_bridge (pid : Nat) :
    SWELib.OS.Memory.readOOMScore pid = .error .ENOSYS  -- Placeholder
