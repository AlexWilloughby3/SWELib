...
/-- Bridge theorem: clone matches specification. -/
theorem clone_bridge (fn : Unit → Unit) (stack : ByteArray) (flags : CloneFlags) (arg : Unit) :
  clone fn stack flags arg = clone_impl fn stack flags arg :=
  by rfl

/-- Bridge theorem: unshare matches specification. -/
theorem unshare_bridge (flags : CloneFlags) :
  unshare flags = unshare_impl flags :=
  by rfl

/-- Bridge theorem: setns matches specification. -/
theorem setns_bridge (fd : NamespaceFD) (nstype : Option CloneFlags) :
  setns fd nstype = setns_impl fd nstype :=
  by rfl

/-- Bridge theorem: get_namespace_fd matches specification. -/
theorem get_namespace_fd_bridge (pid : NamespacedPID) (ns : Namespace) :
