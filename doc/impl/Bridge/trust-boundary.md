...
This document lists all axioms in `impl/SWELibImpl/Bridge/` that represent unproven real-world assumptions.
...
Bridge axioms live in `impl/SWELibImpl/Bridge/`. To audit:

1. Verify every axiom has a `-- TRUST: <issue-url>` comment
2. Check that all referenced issues exist and are open
3. Review the list stays up-to-date
