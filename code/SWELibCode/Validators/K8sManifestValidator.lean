...
def validateObjectMeta (meta : ObjectMeta) : List ValidationError :=
  let errs : List (Option ValidationError) := [
    validateName meta.name |>.map fun m => ⟨"metadata.name", m⟩,
    if meta.namespace.isSome && meta.namespace.get!.isEmpty then
