...
def getResource (cfg : K8sConfig) (apiVersion resource namespace name : String) : IO K8sResult :=
  let path := if namespace.isEmpty
    then s!"/apis/{apiVersion}/{resource}/{name}"
    else s!"/apis/{apiVersion}/namespaces/{namespace}/{resource}/{name}"
...
def listResources (cfg : K8sConfig) (apiVersion resource namespace : String)
    (labelSelector : String := "") : IO K8sResult := do
  let basePath := if namespace.isEmpty
    then s!"/apis/{apiVersion}/{resource}"
    else s!"/apis/{apiVersion}/namespaces/{namespace}/{resource}"
...
def createResource (cfg : K8sConfig) (apiVersion resource namespace body : String) : IO K8sResult :=
  let path := if namespace.isEmpty
    then s!"/apis/{apiVersion}/{resource}"
    else s!"/apis/{apiVersion}/namespaces/{namespace}/{resource}"
...
def updateResource (cfg : K8sConfig) (apiVersion resource namespace name body : String) : IO K8sResult :=
  let path := if namespace.isEmpty
    then s!"/apis/{apiVersion}/{resource}/{name}"
    else s!"/apis/{apiVersion}/namespaces/{namespace}/{resource}/{name}"
...
def patchResource (cfg : K8sConfig) (apiVersion resource namespace name patch : String) : IO K8sResult :=
  let path := if namespace.isEmpty
    then s!"/apis/{apiVersion}/{resource}/{name}"
    else s!"/apis/{apiVersion}/namespaces/{namespace}/{resource}/{name}"
...
def deleteResource (cfg : K8sConfig) (apiVersion resource namespace name : String) : IO K8sResult :=
  let path := if namespace.isEmpty
    then s!"/apis/{apiVersion}/{resource}/{name}"
    else s!"/apis/{apiVersion}/namespaces/{namespace}/{resource}/{name}"
...
def getPod (cfg : K8sConfig) (namespace name : String) : IO K8sResult :=
  getResource cfg "v1" "pods" namespace name

/-- Convenience: list all Pods in a namespace. -/
def listPods (cfg : K8sConfig) (namespace : String) (labelSelector : String := "") : IO K8sResult :=
  listResources cfg "v1" "pods" namespace labelSelector

/-- Convenience: apply a manifest (create or update via server-side apply). -/
def applyManifest (cfg : K8sConfig) (namespace body fieldManager : String) : IO K8sResult :=
  let path := s!"/apis/v1/namespaces/{namespace}/pods?fieldManager={fieldManager}&force=true"
