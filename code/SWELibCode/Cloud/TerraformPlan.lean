...
`SWELibBridge.Oracles.Terraform` trust assumptions to actual
...
private def commonFlags (cfg : TerraformConfig) : Array String :=
  let varFlags := cfg.vars.toArray.concatMap fun (k, v) => #["-var", s!"{k}={v}"]
  let fileFlags := cfg.varFiles.toArray.concatMap fun f => #["-var-file", f]
