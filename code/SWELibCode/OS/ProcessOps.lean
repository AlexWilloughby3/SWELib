...
namespace SWELibCode.OS.ProcessOps
...
def forkProcess : IO (Except Errno PID) := do
  let result ← SWELibCode.Ffi.Syscalls.fork
  match result with
...
def killProcess (target : PID) (sig : Signal) : IO (Except Errno Unit) := do
  let sigNum : UInt32 := match sig with
    ...
    | .SIGSTOP => 19
    | .SIGCONT => 18
  SWELibCode.Ffi.Syscalls.kill target.pid.toInt32 sigNum

/-- Get the current process's PID. -/
def getPid : IO PID := do
  let pid ← SWELibCode.Ffi.Syscalls.getpid
  return ⟨pid.toNatClampNeg⟩

/-- Get the parent process's PID. -/
def getParentPid : IO PID := do
  let pid ← SWELibCode.Ffi.Syscalls.getppid
  return ⟨pid.toNatClampNeg⟩
...
def getEnv (name : String) : IO (Option String) :=
  SWELibCode.Ffi.Syscalls.getenv name

/-- Set an environment variable. -/
def setEnv (name value : String) : IO (Except Errno Unit) :=
  SWELibCode.Ffi.Syscalls.setenv name value 1

/-- Unset an environment variable. -/
def unsetEnv (name : String) : IO (Except Errno Unit) :=
  SWELibCode.Ffi.Syscalls.unsetenv name

/-- Get the current working directory. -/
def getCwd : IO (Except Errno String) :=
  SWELibCode.Ffi.Syscalls.getcwd

/-- Change the current working directory. -/
def changeCwd (path : String) : IO (Except Errno Unit) :=
  SWELibCode.Ffi.Syscalls.chdir path

/-! ## User/group operations -/

/-- Get the real user ID. -/
def getUid : IO UserId := do
  let uid ← SWELibCode.Ffi.Syscalls.getuid
  return ⟨uid.toNat⟩

/-- Get the effective user ID. -/
def getEffectiveUid : IO UserId := do
  let uid ← SWELibCode.Ffi.Syscalls.geteuid
  return ⟨uid.toNat⟩

/-- Get the real group ID. -/
def getGid : IO GroupId := do
  let gid ← SWELibCode.Ffi.Syscalls.getgid
  return ⟨gid.toNat⟩

/-- Get the effective group ID. -/
def getEffectiveGid : IO GroupId := do
  let gid ← SWELibCode.Ffi.Syscalls.getegid
  return ⟨gid.toNat⟩
