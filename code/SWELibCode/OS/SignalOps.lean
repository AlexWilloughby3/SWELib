...
namespace SWELibCode.OS.SignalOps
...
def killTargetToInt32 : KillTarget → Int32
  ...
  | .processGroup pgid => Int32.mk ((0 : UInt32) - pgid.toUInt32)
  | .callerGroup        => Int32.mk 0
  | .allReachable       => Int32.mk ((0 : UInt32) - 1)
...
def sigaction (signum : Signal) (act : Option SignalDisposition) :
  ...
  let (queryOnly, kind, mask, flags) : UInt32 × UInt32 × UInt64 × UInt32 :=
    ...
    | some d =>
      ...
      (0, k, m, f)
  let result ← SWELibCode.Ffi.Syscalls.sigaction_
...
def sigprocmask (how : SigActionHow) (set : Option SigSet) :
  ...
  let result ← SWELibCode.Ffi.Syscalls.sigprocmask_
...
def sigpending : IO (Except Errno SigSet) := do
  let result ← SWELibCode.Ffi.Syscalls.sigpending_
  return result.map maskToSigSet
...
def sendSignal (target : KillTarget) (sig : Option Signal) :
  ...
  SWELibCode.Ffi.Syscalls.kill rawPid rawSig

end SWELibCode.OS.SignalOps
