...
consumed by the C shims in `SWELibCode.Ffi.Memory`.
...
namespace SWELibCode.OS.MemoryOps
...
def mmap (addr : Option VirtualAddress) (length : Nat) (prot : MemoryProtection)
  ...
  let result ← SWELibCode.Ffi.Memory.mmap_
...
def munmap (addr : VirtualAddress) (length : Nat) : IO (Except Errno Unit) :=
  SWELibCode.Ffi.Memory.munmap_ (addrToUInt64 addr) length.toUSize
...
def mprotect (addr : VirtualAddress) (length : Nat) (prot : MemoryProtection) :
  ...
  SWELibCode.Ffi.Memory.mprotect_ (addrToUInt64 addr) length.toUSize (protToUInt32 prot)
...
def brk (addr : VirtualAddress) : IO (Except Errno VirtualAddress) := do
  let result ← SWELibCode.Ffi.Memory.brk_ (addrToUInt64 addr)
  return result.map addrOfUInt64
...
def sbrk (increment : Int) : IO (Except Errno VirtualAddress) := do
  let result ← SWELibCode.Ffi.Memory.sbrk_ increment.toInt64
  return result.map addrOfUInt64
...
def readOOMScore (pid : Nat) : IO (Except Errno OOMScore) := do
  try
    ...
    match content.trim.toNat? with
    | some n => return .ok ⟨n⟩
    | none   => return .error .EIO
  catch _ =>
    return .error .EIO

end SWELibCode.OS.MemoryOps
