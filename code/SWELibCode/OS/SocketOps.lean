...
FFI `UInt32` values. Thin layer over `SWELibCode.Ffi.Syscalls`.
-/

namespace SWELibCode.OS.SocketOps
...
def addressFamilyToUInt32 : AddressFamily → UInt32
  | .AF_INET  => SWELibCode.Ffi.Syscalls.AF_INET
  | .AF_INET6 => SWELibCode.Ffi.Syscalls.AF_INET6
  | .AF_UNIX  => 1  -- AF_UNIX = 1 on both Linux and macOS

/-! ## Socket type conversion -/

def socketTypeToUInt32 : SocketType → UInt32
  | .SOCK_STREAM => SWELibCode.Ffi.Syscalls.SOCK_STREAM
  | .SOCK_DGRAM  => SWELibCode.Ffi.Syscalls.SOCK_DGRAM
...
def socketOptionToLevel (_opt : SocketOption) : UInt32 :=
  SWELibCode.Ffi.Syscalls.SOL_SOCKET

def socketOptionToName : SocketOption → UInt32
  | .SO_REUSEADDR => SWELibCode.Ffi.Syscalls.SO_REUSEADDR
...
def socket (family : AddressFamily) (sockType : SocketType) :
  ...
  SWELibCode.Ffi.Syscalls.socket
